...
def getGenerator(target_size=(64, 64), seed=7, batch_size=4, num_classes=2, parent_dir=None):
    ...
    image_generator = image_datagen.flow_from_directory(parent_dir, target_size=target_size, classes=[img_name],
                                                        ...
                                                        color_mode='rgb')
    label_generator = image_datagen.flow_from_directory(parent_dir, target_size=target_size, classes=[label_name],
                                                        class_mode=None, seed=seed, batch_size=batch_size,
...
def main():
    ...
    from tf.network.Segnet import segnet
    model = segnet(input_shape=(576,576,3),n_labels=2)
...
if __name__ == '__main__':
    print("gup是否可用", tf.test.is_gpu_available(), tf.__version__)
    try:
        if keras.__version__ > '2.2.4':
            print("keras,gpu",keras.backend.tensorflow_backend._get_available_gpus())
        else:
            print('keras可用，但未能检测gpu是否可用')
    except NameError:
        print('没有找到keras')
