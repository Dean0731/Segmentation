...
def transform_common(line_x,line_y):
    ...
    image = tf.image.resize(image,input_size,method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    x = image
...
def transform_double_input(line_x,line_y):
    ...
    image_1 = tf.image.resize(image,input_size,method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
